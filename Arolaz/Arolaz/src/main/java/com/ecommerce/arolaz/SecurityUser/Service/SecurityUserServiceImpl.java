...
//    private boolean phoneConfirmed;
     ...
     */
    @Override
    public Optional<String> signUp(String email, String firstName, String lastName, String phone, String password, String address) {
        LOGGER.info("New user is registering a new account ");
        Optional<SecurityUser> persistUser;
        Optional<String> token = Optional.empty();

        //Explicitly attach roles
        if (!securityUserRepository.findByPhone(phone).isPresent() &&
            ...
            SecurityUser user = new SecurityUser(email,firstName,lastName,phone
                    ,passwordEncoder.encode(password),address, Arrays.asList(role));

//            persistUser = Optional.of(securityUserRepository.save(new SecurityUser(email,name,phone
//                    ,passwordEncoder.encode(password),address, Arrays.asList(role))));
            ...
            //token = Optional.of(jwtProvider.createToken(user,role.get().getRoleName());
...
//        }
    ...
    @Override
    public Optional<SecurityUser> findByPhone(String phone) {
        return securityUserRepository.findByPhone(phone);
    }

    @Override
    public Optional<SecurityUser> findByFirstName(String firstName) {
        return securityUserRepository.findByFirstName(firstName);
    }
    ...
    public Optional<String> signIn(String requiredEntry, String password) {
        ...
        if (isNumeric(requiredEntry)) {
            user = securityUserRepository.findByPhone(requiredEntry);
        } else {
        ...
        if (user.isPresent()) {
            ...
            if(isNumeric(requiredEntry)){
                user = securityUserRepository.findByPhone(userDetails.getUsername());
            }
            else{
    ...
    public String extractPhoneFromToken(String token) {
        ...
        return securityUserRepository.findByPhone(phone).get().getPhone();
    }
